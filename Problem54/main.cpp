...
int main()
{
    ...
    long long count=0;
    int itrs=0;
    QFile readFile("C:\\Users\\andrew\\Downloads\\p054_poker.txt");
    if(readFile.open(QIODevice::ReadOnly)) {
        const auto &fileString = QString(readFile.readAll());
        readFile.close();
        auto lines = fileString.split("\n",QString::SkipEmptyParts);
        for(const auto &line : lines) {
            auto handsSplit = line.split(" ",QString::SkipEmptyParts);
            std::vector<int> scores(2);
            std::vector<std::map<int,int>> valsToCounts(2);
            std::vector<std::map<QCharRef,int>> suitsToCounts(2);
            std::vector<std::vector<QString>> hands(2,std::vector<QString>(5));
            for(int i=0; i<2; ++i) {
                for(int j=0; j<5; ++j) {
                    int offset = i==0 ? 0 : 5;
                    hands[i][j] = handsSplit[j+offset];
                    auto val = charValueToInt(hands[i][j][0].toLatin1());
                    if(valsToCounts[i].count(val) == 0) {
                        valsToCounts[i][val] = 1;
                    }
                    else {
                        ++valsToCounts[i][val];
                    }
                    auto suit = hands[i][j][1];
                    if(suitsToCounts[i].count(suit) == 0) {
                        suitsToCounts[i][suit] = 1;
                    }
                    else {
                        ++suitsToCounts[i][suit];
                    }
                }
                scores[i] = scoreHand(valsToCounts[i],suitsToCounts[i]);
            }
    ...
    qDebug() << "player one wins " << count << " out of " << itrs << " games";
